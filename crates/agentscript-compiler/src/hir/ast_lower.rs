...
impl<'a, 'sess> LowerCtx<'a, 'sess> {
    ...
    fn script_declaration(&self, decl: &ScriptDeclaration) -> Result<HirDeclaration, HirLowerError> {
        match decl.kind {
            ...
            ScriptKind::Library => Ok(HirDeclaration::Library),
        }
    }
    ...
    fn lower_call(
        ...
        if path.as_slice() == ["plot"] {
            if args.is_empty() {
                return Err(HirLowerError::at(expr_span, "plot needs at least one argument"));
            }
            let inner = self.lower_expr(&args[0].1)?;
            let title = plot_title_from_args(args);
            return Ok(self.alloc_expr(
                HirExpr::Plot {
                    expr: inner,
                    title,
...
fn plot_title_from_args(args: &[(Option<String>, Expr)]) -> Option<String> {
    args.iter()
...
"#;
    ...
    #[test]
    fn assign_plot_rhs_lowers_to_hir_plot_expr() {
...
"#;
        ...
        check_script(&script).expect("check");
        let c = crate::analyze_to_hir_compiler(&script).expect("hir");
        let hir = c.session.hir.as_ref().expect("hir");
        let found = hir.body.iter().any(|s| {
            matches!(
                s,
                HirStmt::Let { value, .. }
                    if matches!(&hir.exprs[value.0 as usize], HirExpr::Plot { .. })
            )
        });
        assert!(found, "expected Let binding to HirExpr::Plot, body={:?}", hir.body);
    }
}
