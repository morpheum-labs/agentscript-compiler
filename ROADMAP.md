...
| **Imports / exports** | `import … as`, `export` fn/var | **Partial** | Host links a `library()` unit via [`register_import_library`](crates/agentscript-compiler/src/lib.rs) on [`CompilerSession`](crates/agentscript-compiler/src/session.rs) before analyzing the consumer; then qualified calls to **exported functions** typecheck ([`typecheck.rs`](crates/agentscript-compiler/src/semantic/passes/typecheck.rs)). Bare alias uses [`HirType::ImportNamespace`](crates/agentscript-compiler/src/hir/ty.rs). **HIR** still rejects lowered calls through a linked library ([`ast_lower.rs`](crates/agentscript-compiler/src/hir/ast_lower.rs)). Duplicate import alias ([`early.rs`](crates/agentscript-compiler/src/semantic/passes/early.rs)); lexical + resolver roots ([`lexical.rs`](crates/agentscript-compiler/src/semantic/passes/lexical.rs), [`resolver.rs`](crates/agentscript-compiler/src/semantic/passes/resolver.rs)); `import` lines skipped as metadata in HIR. See `docs/aether-integration-gap.md` §2.1. **No** TV registry or WASM inlining of libraries yet. |
...
- [x] **Example script coverage:** [`examples/uptrend.pine`](examples/uptrend.pine) (Pine v6–style source) is exercised by **`parse_and_analyze`** in [`parse_smoke.rs`](crates/agentscript-compiler/tests/parse_smoke.rs) (`examples_uptrend_pine_parse_and_analyze`). [`examples/weighted_strategy_braced_slice.pine`](examples/weighted_strategy_braced_slice.pine) is a small **braced** excerpt (strategy header, `=>` helper with `MAType`-style names, `ta.sma`, braced `if`) covered by `examples_weighted_strategy_braced_slice_parse_and_analyze`. The full TV-indented source [`examples/weighted_strategy.pine`](examples/weighted_strategy.pine) remains a larger manual reference (not CI-required as-is). WASM unit tests still use a **minimal indicator** string because full real-world scripts need additional `ta.*` / series host surface (e.g. `ta.tr`, `ta.atr`, non-`close` series in guest ABI) before `compile_script_to_wasm_v0` can succeed end-to-end.
