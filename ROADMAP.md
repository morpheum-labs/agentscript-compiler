...
| **Array literals** | `[a, b]`, `[]` | **Partial** | [`ExprKind::Array`](crates/agentscript-compiler/src/frontend/ast/expr.rs); homogeneous typing + promotion in [`typecheck.rs`](crates/agentscript-compiler/src/semantic/passes/typecheck.rs). HIR lowering for array values still **None** (no `HirExpr` array aggregate yet). |
...
| **Assignments** | `=` first assign vs `:=` reassignment | **Partial** | Pine-style first `=` introduces binding (lexical + typecheck + HIR). Compound `+=` … `/=` lowered to `Let` + binary in [`ast_lower.rs`](crates/agentscript-compiler/src/hir/ast_lower.rs). `:=` and tuple assign: typecheck partial; HIR for `:=` **None**. |
...
| **`input.*` factory fns** | `input.int`, `input.float`, … | **Partial** | `input.int` / `input.float` literal defaults in assign / `input` decls handled in HIR + WASM ([`BuiltinKind::InputInt`](crates/agentscript-compiler/src/hir/builtin.rs) / `InputFloat`). Other `input.*` factories not modeled. |
...
- [x] **Guest module ABI v0 (partial):** import indices and export names are defined in [`hir_wasm.rs`](crates/agentscript-compiler/src/codegen/hir_wasm.rs) and described in [`aether/docs/agentscript-guest-abi.md`](../../aether/docs/agentscript-guest-abi.md). **Still open for Phase 2 “done”:** full `request.*` / strategy surfaces in HIR+WASM, complete user-fn + library coverage, richer `init`/`step` signatures, and Aether/MWVM contract tests beyond the current smoke checks.
