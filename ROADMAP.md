...
| **`request.financial`** | Financial series by id/period | **Partial** | Typecheck + HIR [`FinancialCall`](crates/agentscript-compiler/src/hir/financial.rs); WASM v0: `aether.request_financial` (string literals; optional `ignore_invalid_symbol` **bool literal** only). Pine v6 `gaps` / `currency` / dynamic args still open. |
...
1. **Full span coverage** — replace `Span::DUMMY` on semantic/codegen error paths where the AST or HIR has a real range (progress: [`hir_ty_to_val`](crates/agentscript-compiler/src/codegen/hir_wasm.rs) / wasm type errors use the expression span; grep `Span::DUMMY` under `crates/agentscript-compiler` for remaining sites).
...
3. **`request.security` / `request.financial` end-to-end** — security: typecheck + HIR + WASM `request_security` + MWVM stub; financial: typecheck + HIR + WASM `request_financial` + MWVM stub (**v0** literal subset). Remaining: full Pine v6 params, dynamic symbols, real host data + golden tests.
...
- [x] **`request.financial` lowering (v0):** HIR + WASM import `request_financial` + MWVM stub; string pool + 7×`i32` args. **Still open:** full Pine v6 surface (`gaps`, `currency`, …), non-literal args, determinism / prefetch aligned with `request.security`.
