...
| **Imports / exports** | `import … as`, `export` fn/var | **Partial** | Duplicate import alias ([`early.rs`](crates/agentscript-compiler/src/semantic/passes/early.rs)); import aliases in lexical root + valid dotted roots ([`lexical.rs`](crates/agentscript-compiler/src/semantic/passes/lexical.rs), [`resolver.rs`](crates/agentscript-compiler/src/semantic/passes/resolver.rs)); qualified `alias.*` / `alias.fn(...)` rejected in typecheck until library linking exists ([`typecheck.rs`](crates/agentscript-compiler/src/semantic/passes/typecheck.rs)); `import` items are skipped in HIR ([`ast_lower.rs`](crates/agentscript-compiler/src/hir/ast_lower.rs)). Export forms share early / checker paths with non-export counterparts. **No** module graph or cross-unit symbol linking. |
